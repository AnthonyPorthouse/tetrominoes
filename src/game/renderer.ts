...
function renderMenu(time: Ticker, app: Application, game: Game) {
  ...
  if (currentStartText) {
    (currentStartText as Text).style.fontSize =
      16 + 16 * Math.abs(Math.sin(currentScale));
  } else {
    const text = new Text({
      text: "Click to Start",
      style: {
...
function renderGame(_: Ticker, app: Application, game: Game) {
  ...
  board.getChildrenByLabel("block").flatMap((el) => el.destroy());
  ...
  for (let block of Object.values(tetromino.blocks)) {
    const sprite = Sprite.from("tetromino.webp");
    sprite.position.set(
      (block.point.x + position.x) * sprite.width,
      (block.point.y + position.y) * sprite.height,
    );
    sprite.tint = block.color;
    sprite.label = "block";
    board.addChild(sprite);
  ...
  const nextBlocks = Object.values<Block>(
    game.state.blockBag.previewNext().blocks,
  ).map((block) => {
    const sprite = Sprite.from("tetromino.webp");
    sprite.position.set(
      block.point.x * sprite.width,
      block.point.y * sprite.height,
    );
    return sprite;
  });
  ...
  if (game.state.held) {
    const heldBlocks = Object.values<Block>(game.state.held.blocks).map(
      (block) => {
        const sprite = Sprite.from("tetromino.webp");
        sprite.position.set(
          block.point.x * sprite.width,
